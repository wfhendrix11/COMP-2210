...
public class Extractor {
   ...
   public SortedSet<Line> getLinesFast() {
      ...
      for (int i = 0; i < sortedPoints.length; i++) {
         slopes[i] = sortedPoints[0].slopeTo(sortedPoints[i]);
      }
      
      // find points with the same slope 
      for (int i = 0; i < sortedPoints.length; i++) {
         double slope1 = sortedPoints[0].slopeTo(sortedPoints[i]);
         double slope2 = sortedPoints[0].slopeTo(sortedPoints[i + 1]);
